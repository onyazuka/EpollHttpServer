...
inline std::string statusText(size_t code)
{
	switch (code)
	{
...
util::web::http::HttpResponse HttpServer::callRoute(const std::string& route, const util::web::http::HttpRequest& request) const {
	...
	return defaultReponse(404);
}

util::web::http::HttpResponse HttpServer::defaultReponse(size_t statusCode) const {
	util::web::http::HttpResponse response;
	response.status = statusCode;
	response.statusText = statusText(statusCode);
